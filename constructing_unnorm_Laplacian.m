% Constructs the unnormalized Laplacian matrix and takes the "K_1" smallest
% eigenvectors and gives the matrix U to Kmeans script and gets back 
% clusters
...
SF1_EuclideanSqrdSim = true; % SF1: SQUARED EUCLIDEAN DISTANCE SIMILARITY FUNCTION WITH SIM THRESHOLD
SF2_AveDiffSim = false; % SF2: Average of maximum sampled differences similarity function
...
for i_class = [Cluster_pairs(a_b,1) Cluster_pairs(a_b,2)]
    ...
    [Eigen_vectors, Eigen_values] = eigs(L,K_1,'smallestabs');
    U = Eigen_vectors;
    Udim = size(U);
