% Constructs the normalized random walk Laplacian matrix and takes the 
% "K_1" smallest eigenvectors and gives the matrix U to Kmeans script and 
% gets back clusters
...
for i_class = [Cluster_pairs(a_b,1) Cluster_pairs(a_b,2)]
    ...
    Lrw = Dinv*L; % Normalised Random Walk Laplacian

    [Eigen_vectors, Eigen_values] = eigs(Lrw,K_1,'smallestabs');
    U = Eigen_vectors;
    Udim = size(U);
